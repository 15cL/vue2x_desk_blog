    ...
    <div class="row">
      <div class="col_fir">
        <transition name="banner">
          <div class="banner" v-if="bannerFlag && articleFlag">
      ...
      <div class="col_sec">
        <transition name="user">
          <div class="user" v-if="bannerFlag && articleFlag">
            <img
...
export default {
  ...
  data () {
    return {
      ...
      tags: [],
      articles: [],
      bannerFlag: this.$route.path !== '/blog',
      articleFlag:
        this.$route.path !== '/blog/detail' &&
        this.$route.path !== '/blog/cate',
      hotArticles: [],
      refresh: true
    }
  },
  watch: {
    $route (to, from) {
      this.bannerFlag = !to.path.match(/^\/blog.*/)
      this.articleFlag =
        to.path !== '/blog/detail' && this.$route.path !== '/blog/cate'

      // 切换文章刷新
      if (to.query.article !== from.query.article || to.query.cate !== from.query.cate) {
        this.refresh = false
        this.$nextTick(() => {
          this.refresh = true
        })
