...
(function (global, factory) {
  ...
  function addHandler (
    ...
    const newHandler = rangeSetItem({ value: value.trim(), dynamic: dynamic }, range)
  ...
  const isNotTextNode = function (c) { return c.tag || isAsyncPlaceholder(c) }

  const isVShowDirective = function (d) { return d.name === 'show' }
  ...
  function parseHTML (html, options) {
    ...
    while (html) {
      ...
      if (!lastTag || !isPlainTextElement(lastTag)) {
        ...
        let text = (void 0); let rest = (void 0); let next = (void 0)
        if (textEnd >= 0) {
  ...
  function genScopedSlots (
    ...
    let needsForceUpdate = el.for || Object.keys(slots).some(function (key) {
